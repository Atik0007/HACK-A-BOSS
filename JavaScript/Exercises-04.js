...
// 2. Obtén la suma total de todas las edades de las personas francesas.
const totalAgeFR = persons.reduce((total, person) => {
  if (person.country === 'FR') {
    total += person.age;
  }
  return total;
}, 0);
console.log(totalAgeFR);
...
// 5. Obtén un array con los coches de los españoles.
const carsOfSpain = persons.filter((person) => {
  return cars.find((car) => person.country === 'ES' && car.id === person.car);
});
console.log(carsOfSpain);
